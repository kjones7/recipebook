...
public class RecipeIngredientResource {
    ...
    public List<RecipeIngredient> getAllRecipeIngredients(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all RecipeIngredients");
        if (eagerload) {
            return recipeIngredientRepository.findAllWithEagerRelationships();
        } else {
